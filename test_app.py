...
def test_index_get(client, sample_books):
    ...
    # Check for presence of "Book List" in the rendered template
...
def test_index_post_search(client, sample_books):
    ...
    assert response.status_code == 200
    # Check if only matching book is returned (Hitchhiker's Guide)
...
def test_edit_book_existing(client, sample_books):
    ...
    # Check if the first book's title was updated
